...
def git_push_hook(webhookParser: WebHookDataParser, handler: IssueHandler):
    data = request.data
    if len(data) == 0:
    ...
    try:
        ref = webhookParser.parse(event_request)
    except ParseError:
        return make_response("", 200)

    search = re.search(TRACKED_BRANCH_REGEXP, ref.ref_id)
    if (ref.update_type == "UPDATE") & (search is not None) & (ref.repo_name in WHITE_LISTED_REPOS):
        logger.info("Submit commit for processing")
        __git_pool__.submit(process_hook_data, ref, handler)
    else:
        logger.info("This branch {} is not tracked".format(ref.ref_id))
